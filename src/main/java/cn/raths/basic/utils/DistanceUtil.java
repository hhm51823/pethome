...
public class DistanceUtil {
    public static  Point getPoint(String address){
        String Application_ID="Pv2npEKEuuLppLGAQmF8MUQBNV5O4MAg";//配置上自己的百度地图应用的AK
        try{
        ...
        if (shops.size()>1){
            for (int i=1;i<shops.size();i++){
                if (getDistance(point,getPoint(shops.get(i).getAddress()))<distance){
                    shop=shops.get(i);//替换更近的店铺
                    distance = getDistance(point,getPoint(shops.get(i).getAddress()));//重新赋值最近距离
                }
            }
        }
        if(distance <=50000){//50公里
            return shop;//50公里内有店铺
        }
        return null;//50公里内没有店铺
    }
